...
{
    /// <summary>
    /// Defines how a <see cref="KBLog"/> is formatted when logged by a <see cref="IKBLogger"/>.
    /// </summary>
    [Serializable]
    public struct KBLogFormatter : IKBLogFormatter
    {
        #region Delegate Definitions
        /// <summary>
        /// A delegate for constructing a prefix of a <see cref="KBLog"/>.
        /// </summary>
        /// <param name="log">The log to read from.</param>
        /// <returns>A prefix.</returns>
        public delegate string PrefixFormatter(KBLog log);

        /// <summary>
        /// A delegate for formatting a message of a <see cref="KBLog"/>.
        /// </summary>
        /// <param name="log">The log to read from.</param>
        /// <returns>A formatted message.</returns>
        public delegate string MessageFormatter(KBLog log);

        /// <summary>
        /// A delegate for applying color to a formatted <see cref="KBLog"/> message.
        /// </summary>
        /// <param name="log">The log to read from.</param>
        /// <param name="message">The message to color.</param>
        /// <returns>A colored version of the passed message.</returns>
        public delegate string ColorSetter(KBLog log, string message);

        /// <summary>
        /// A delegate for composing the final message of a <see cref="KBLog"/>.
        /// </summary>
        /// <param name="log">The log to read from.</param>
        /// <param name="formatter">The formatter to use for constructing the message.</param>
        /// <returns>The final formatted message.</returns>
        public delegate string CompositorFunction(KBLog log, KBLogFormatter formatter);
        #endregion
        ...
        /// </summary>
        public static KBLogFormatter Empty => new();
        #endregion

        /// <summary>
        /// Constructs a new KBLogFormatter.
        /// </summary>
        /// <param name="formatPrefix">The delegate for constructing the prefix of a log. Defaults to <see cref="KBLogFormats.DefaultPrefix"/></param>
        /// <param name="formatMessage">The delegate for formatting the message of a log. Defaults to <see cref="KBLogFormats.DefaultMessage"/></param>
        /// <param name="setColor">The delegate for applying color to a formatted message. Defaults to <see cref="KBLogFormats.DefaultColor"/></param>
        /// <param name="compositor">The delegate for composing the final formatted message. Defaults to <see cref="KBLogFormats.DefaultCompositor"/></param>
        public KBLogFormatter(PrefixFormatter formatPrefix = null, MessageFormatter formatMessage = null, ColorSetter setColor = null, CompositorFunction compositor = null)
        {
            formatPrefix ??= KBLogFormats.DefaultPrefix;
            formatMessage ??= KBLogFormats.DefaultMessage;
            setColor ??= KBLogFormats.DefaultColor;
            compositor ??= KBLogFormats.DefaultCompositor;
        ...
        public readonly string Format(KBLog log, [CallerFilePath] string callerFilePath = "", [CallerLineNumber] long callerLineNumber = -1)
        {
        ...
        #region Operators
        public static bool operator==(KBLogFormatter a, KBLogFormatter b)
        {
