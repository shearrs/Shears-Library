...
mergeInto(LibraryManager.library, {
  _global: null,
  init__deps: ["_global", "saveAs", "corsEnabled", "download", "click", "bom"],
  init: function () {
    ...
    _saveAs =
      __global.saveAs ||
      (typeof window !== "object" || window !== __global
        ? function saveAs() {}
        ...
        : function saveAs(blob, name, opts, popup) {
            ...
            if (typeof blob === "string") return _download(blob, name, opts);
  ...
  UNITY_SAVE_BYTEARRAY: function (arr, size, name, mimetype) {
    ...
    var blob = new Blob([bytes], { type: UTF8ToString(mimetype) });
    saveAs(blob, UTF8ToString(name));
  },
