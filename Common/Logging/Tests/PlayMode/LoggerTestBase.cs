...
namespace Shears.Logging.Tests
{
    public abstract class LoggerTestBase
    {
        ...
        public virtual void PrefixTest()
        {
            ...
            ValidateOutput(
                new("Hello World!", prefix: "Custom Prefix", level: SHLogLevels.Verbose)
            );
            ValidateOutput(
                new("Hello World!", prefix: "Custom Prefix", level: SHLogLevels.Warning)
            );
        ...
        public virtual void KBFormatterTest()
        {
            ...
            var formatPrefix = SHLogFormats.CombinePrefixes(
                " ",
                SHLogFormats.ContextPrefix,
                formatPrefixPart
            );
            ...
            var customFormatter = new SHLogFormatter(
                formatPrefix,
                formatMessage,
                setColor,
                compositor
        ...
        private void ValidateOutput(SHLog log, ISHLogFormatter formatter = null)
        {
            ...
            if (formatter == null || !formatter.IsValid())
            {
                ...
                if (
                    ...
                    && (log.Level & SHLogLevels.Error) != 0
                )
                    LogAssert.Expect(targetMessage);
        ...
        protected string FormatPrefixWithIncrementedLineNumber(
            SHLog log,
            SHLogFormatter.PrefixFormatter prefixFormatter
        )
        {
            ...
            return defaultPrefix[..lineNumberStartIndex]
                + lineNumber
                + defaultPrefix[lineNumberEndIndex..];
        }
        ...
        private ISHLogFormatter FixLineNumberPrefix(ISHLogFormatter formatter)
        {
            if (formatter is SHLogFormatter structFormatter)
            {
                ...
                structFormatter.FormatPrefix = (log) =>
                    FormatPrefixWithIncrementedLineNumber(log, currentPrefixFormatter);
            ...
            else if (formatter is SHLogFormatterData dataFormatter)
            {
                ...
                dataFormatter.PrefixFormatter = (log) =>
                    FormatPrefixWithIncrementedLineNumber(log, currentPrefixFormatter);
            ...
            Debug.LogWarning(
                $"Need to fix line number for unsupported formatter!: {formatter.GetType().Name}"
            );
            return null;
        }
    }
